...
function buildReactTree(root){
    ...
    if (TestUtils.isDOMComponent(root)) {
    ...
    else if (TestUtils.isCompositeComponent(root)) {
...
function findAllElementsWithSelector(root, rule, omitRootNode){
    var matches = TestUtils.findAllInRenderedTree(root, elementRuleMatcher(root, rule, omitRootNode)),
        subRule = rule.rule;
    if(subRule){
        var subMatches = [];
 ...
 */
var RenderWrapper = createReactClass({
    render: function(){
...
module.exports = {
    ...
    getRouterComponent: function(Router, reactComponent, props, path) {
        ...
        Router.run(routes, loc, function (Handler) {
            ...
            component = TestUtils.findRenderedComponentWithType(mainComponent, reactComponent);
        });
    ...
    renderPartial(partial){
        return TestUtils.renderIntoDocument(<RenderWrapper>{partial}</RenderWrapper>);
    },
    ...
    findComponentCountWithClassname: function(root, className, count){
        ...
        var matches = TestUtils.scryRenderedDOMComponentsWithClass(root, className);
    ...
    findComponentCountWithTag: function(root, tagName, count){
        ...
        var matches = TestUtils.scryRenderedDOMComponentsWithTag(root, tagName);
        if(matches.length === count){
            return true;
        }
