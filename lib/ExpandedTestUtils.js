define(function(require) {
    ...
    function validateParsedCssRule(rule){
        if(rule.pseudos){
            throw new Exception("Pseudo selectors are currently not available. They also don't make sense in React trees.");
        }
    ...
    return {
        ...
        getRouterComponent: function(reactComponent, props, path) {
            ...
            var routes = React.createElement(Route, {name: path, path: "user/:userID", handler: reactComponent});
        ...
        /**
         * Determines if the number of elements found with the provided class name is equal to the
